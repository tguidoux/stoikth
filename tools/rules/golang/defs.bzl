...
def stoikth_go_test(name, **kwargs):
    """
...
def stoikth_go_library(name, **kwargs):
    """
...
def stoikth_go_binary(name, **kwargs):
    """
...
def _go_layers(name, binary):
    ...
    we could improve this in the future similar to how the `stoikth_py_image` macro
    divides up layers.

    Args:
        name: Prefix for generated targets, to ensure they are unique within the package.
        binary: The name of the stoikth_go_binary to bundle in the container.
...
def stoikth_go_image(name, binary, image_tags, include_runfiles = True, tars = [], base = None, entrypoint = None, **kwargs):
    """
